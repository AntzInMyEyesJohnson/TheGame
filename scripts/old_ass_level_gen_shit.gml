...
for(var yy = 0;yy < height;yy++;){for(var xx = 0; xx < width;xx++;){
    if (grid[# xx, yy] == FLOOR){
        ...
        tile_add(path_tile_type,CELL_WIDTH*choose(0,1,2),CELL_HEIGHT*choose(0,1),CELL_WIDTH,CELL_HEIGHT,xx*CELL_WIDTH,yy*CELL_HEIGHT,900);
        
        //draw foliage (grass,bushes)
        if (irandom(foliage_odds) == foliage_odds)
            {tile_add(foliage_tile_type,CELL_WIDTH*0,CELL_HEIGHT*0,32,64,xx*CELL_WIDTH+choose(0,4,8,12),yy*CELL_HEIGHT+choose(0,4,8,12),-(yy*CELL_HEIGHT+CELL_HEIGHT/2));}
        
        if (global.area != HUBMAP)
            {spawn_enemies(xx,yy);}
...
for (var yy = 0; yy < height*2; yy++;) {for (var xx = 0; xx < width*2; xx++;) {        
    if (grid[# xx div 2, yy div 2] == FLOOR or grid[# xx div 2, yy div 2] == DOOR) {
        // Get the tile's x and y
        var tx = xx*tw;
        var ty = yy*th;
        
        var right = grid[# (xx+1) div 2, yy div 2] != FLOOR and grid[# (xx+1) div 2, yy div 2] != DOOR;
        var left = grid[# (xx-1) div 2, yy div 2] != FLOOR and grid[# (xx-1) div 2, yy div 2] != DOOR;
        var top = grid[# xx div 2, (yy-1) div 2] != FLOOR and grid[# xx div 2, (yy-1) div 2] != DOOR;
        var top2 = grid[# xx div 2, (yy-2) div 2] != FLOOR and grid[# xx div 2, (yy-2) div 2] != DOOR;
        var bottom = grid[# xx div 2, (yy+1) div 2] != FLOOR and grid[# xx div 2, (yy+1) div 2] != DOOR;
        
        var top_right = grid[# (xx+1) div 2, (yy-1) div 2] != FLOOR and grid[# (xx+1) div 2, (yy-1) div 2] != DOOR;
        var top_right2 = grid[# (xx+1) div 2, (yy-2) div 2] != FLOOR and grid[# (xx+1) div 2, (yy-2) div 2] != DOOR;
        var top_left = grid[# (xx-1) div 2, (yy-1) div 2] != FLOOR and grid[# (xx-1) div 2, (yy-1) div 2] != DOOR;
        var top_left2 = grid[# (xx-1) div 2, (yy-2) div 2] != FLOOR and grid[# (xx-1) div 2, (yy-2) div 2] != DOOR;
        var bottom_right = grid[# (xx+1) div 2, (yy+1) div 2] != FLOOR and grid[# (xx+1) div 2, (yy+1) div 2] != DOOR;
        var bottom_left = grid[# (xx-1) div 2, (yy+1) div 2] != FLOOR and grid[# (xx-1) div 2, (yy+1) div 2] != DOOR;
        ...
        if (right) {
            if (bottom) {
                ...
                if (top_right) {
                    ...
                    tile_add(wall_tile_type_rare, tw*4, th*0, tw, th, tx+tw, ty-th, -ty);
                } else {
                    tile_add(wall_tile_type, tw*3, th*0, tw, th, tx, ty-th, -ty);
                    tile_add(wall_tile_type_rare, tw*3, th*0, tw, th, tx, ty-th, -ty);
                }
        ...
        if (left) {
            if (bottom) {
                ...
                if (top_left) {
                    ...
                    tile_add(wall_tile_type_rare, tw*3, th*0, tw, th, tx-tw, ty-th, -ty);
                } else {
                    tile_add(wall_tile_type, tw*4, th*0, tw, th, tx, ty-th, -ty);
                    tile_add(wall_tile_type_rare, tw*4, th*0, tw, th, tx, ty-th, -ty);
                }
        ...
        if (top) {
            var odds = 5;
            var wall = wall_tile_type;
            if (irandom(odds) == odds){wall = wall_tile_type_rare;}
            if (!top_right) {
                tile_add(wall, tw*2, th*2, tw, th, tx, ty-th, -ty);
            } else if (!top_left) {
                tile_add(wall, tw*0, th*2, tw, th, tx, ty-th, -ty);
            } else {
                //double the size for top tiles
                tile_add(wall, tw*1, th*1, tw, th*2, tx, ty-2*th, -ty);
            }
            //for shadows CHANGE TO TILE
            tile_add(bg_wall_shadow,0,0,32,32,tx,ty,500);
